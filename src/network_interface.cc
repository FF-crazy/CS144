...
void NetworkInterface::send_datagram( const InternetDatagram& dgram, const Address& next_hop )
{
  ...
  if ( next_mac != ip_to_eth_.end() ) {
    ...
    if ( arp_sent == arp_request_time_.end() || current_time_ - arp_sent->second >= ARP_REQUEST_TIMEOUT ) {
      ...
      EthernetFrame arp_frame { { ETHERNET_BROADCAST, ethernet_address_, EthernetHeader::TYPE_ARP },
                                serialize( arp ) };
      transmit( arp_frame );
  ...
  if ( frame.header.type == EthernetHeader::TYPE_ARP ) {
    ...
    if ( arp.opcode == ARPMessage::OPCODE_REQUEST && arp.target_ip_address == ip_address_.ipv4_numeric() ) {
      EthernetFrame reply_frame {
        { arp.sender_ethernet_address, ethernet_address_, EthernetHeader::TYPE_ARP },
        serialize( make_ARP_( ARPMessage::OPCODE_REPLY, arp.sender_ip_address, arp.sender_ethernet_address ) ) };
      ...
      if ( pending != pending_datagram_.end() ) {
        for ( const auto& dgram : pending->second ) {
          EthernetFrame data_frame { { arp.sender_ethernet_address, ethernet_address_, EthernetHeader::TYPE_IPv4 },
                                     serialize( dgram ) };
          transmit( data_frame );
        }
